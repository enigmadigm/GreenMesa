...
module.exports = {
    ...
    async execute(client, message, args, conn) {
        fetch(`https://opentdb.com/api.php?amount=1&difficulty=easy&type=multiple&encode=url3986`)
            .then(res => res.json())
            .then(async j => {
                ...
                if (Math.floor(Math.random() * 10) >= 9) {
                    message.channel.send('This command is currently under development, any feedback is appreciated.\nFeatures like Questions by Category, Continuous Game, and Difficulty Options\ncan be expected in the future.')
                }
                ...
                collector.on('end', async (collected, reason) => {
                    if (reason == 'time') {
                        ...
                        return message.channel.send('Looks like nobody got the answer this time.');
                    }
                    triviaMessage.embeds[0].color = 65280;
                    triviaMessage.embeds[0].description = triviaChoices.map((e, i) => {
                        if (i != correctIndex) return `🟥${e}`;
