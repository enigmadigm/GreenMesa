...
interface AntiGifProps extends HomeProps {
...
export function AntiGif(props: AntiGifProps) {
    const { setStatus, handleModuleSave } = props;
    const [loaded, setLoaded] = React.useState(false);
    const [unsaved, setUnsaved] = React.useState(false);
    const [mod, setMod] = React.useState<AutomoduleData>({ name: "antigif", text: false, enableAll: false, applyRoles: [], roleEffect: 'ignore' });
    const [original, setOriginal] = React.useState<AutomoduleData>({ name: "antigif", text: false, enableAll: false, applyRoles: [], roleEffect: 'ignore' });

    React.useEffect(() => {
        fetch(`/api/discord/guilds/${props.meta.id}/automod/antigif`)
            .then(x => x.json())
    ...
    const handleSaveClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        try {
            ...
            handleModuleSave("antigif", stringMod, setLoaded);
        } catch (error) {
            console.error(error);
        }
    }
    ...
    return loaded ? (
        <div className="x-card">
            ...
            <div className="x-card-header">Anti Gif</div>
            <div className="x-card-body">
                <h5 className="cardsubtitle">Module Configuration</h5>
                <p style={{ marginBottom: "1rem" }}>This module will delete all messages that have GIFs (the most common animated image format) <strong>attached</strong> to them.</p>
                <hr style={{ marginTop: 10, marginBottom: 15 }} />
                <FormControl display="flex" alignItems="center">
                    <FormLabel htmlFor="enable-antigif" mb="0">
                        Enable everywhere? (overrides channel selection)
                    </FormLabel>
                    <Switch id="enable-antigif" onChange={(e) => handleEnableAllToggle(e)} defaultChecked={mod.enableAll} style={{ marginRight: 10 }} />
