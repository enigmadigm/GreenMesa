...
interface AntiEmbedProps extends HomeProps {
...
export function AntiEmbed(props: AntiEmbedProps) {
    const { setStatus, handleModuleSave } = props;
    const [loaded, setLoaded] = React.useState(false);
    const [unsaved, setUnsaved] = React.useState(false);
    const [mod, setMod] = React.useState<AutomoduleData>({ name: "antiembed", text: false, enableAll: false, applyRoles: [], roleEffect: 'ignore' });
    const [original, setOriginal] = React.useState<AutomoduleData>({ name: "antiembed", text: false, enableAll: false, applyRoles: [], roleEffect: 'ignore' });

    React.useEffect(() => {
        fetch(`/api/discord/guilds/${props.meta.id}/automod/antiembed`)
            .then(x => x.json())
    ...
    const handleSaveClick = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        try {
            ...
            handleModuleSave("antiembed", stringMod, setLoaded);
        } catch (error) {
            console.error(error);
        }
    }
    ...
    return loaded ? (
        <div className="x-card">
            ...
            <div className="x-card-header">Anti Embed</div>
            <div className="x-card-body">
                ...
                <FormControl display="flex" alignItems="center">
                    <FormLabel htmlFor="enable-antiembed" mb="0">
                        Enable everywhere? (overrides channel selection)
                    </FormLabel>
                    <Switch id="enable-antiembed" onChange={(e) => handleEnableAllToggle(e)} defaultChecked={mod.enableAll} style={{ marginRight: 10 }} />
